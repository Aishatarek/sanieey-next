...
const Page = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                ...
                const response = await fetch('https://sani3ywebapiv1.runasp.net/api/CraftsmanWorks/GetAll', {
                    method: 'GET',
                    headers: {
                        'accept': '*/*',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let url = 'https://sani3ywebapiv1.runasp.net/api/CraftsmanWorks/upload-previous-work'
            let method = 'POST'
            
            if (editingId) {
                url = `https://sani3ywebapiv1.runasp.net/api/CraftsmanWorks/update/${editingId}`
                method = 'PUT'
            }
            ...
            const updatedResponse = await fetch('https://sani3ywebapiv1.runasp.net/api/CraftsmanWorks/GetAll', {
                headers: {
                    'accept': '*/*',
                    'Authorization': `Bearer ${authToken}`
                }
            ...
            alert(editingId ? 'تم تحديث العمل بنجاح' : 'تم إضافة العمل بنجاح')
        } catch (error) {
    ...
    const handleEdit = (project: Project) => {
        setFormData({
            ...
            dateJobDone: project.dateJobDone.split('T')[0], // Format date for input
            pictures: []
        })
        setPreviewImages(project.pictures || [])
                            ...
                            <div id="info3" className="info-content">
                                <div className="lk">
                                    <div className="add-work">
                                        <div className="cx">
                                            <span className="span-add">الأعمال السابقة</span>
                                        ...
                                        <div>
                                            <Link href="/sanieey-dashboard/new-project">
                                            <button 
                                                className="btn-pluss"
                                ...
                                {loading ? (
                                    <p className="text-center py-8">جاري تحميل الأعمال...</p>
                                ) : error ? (
                                    <p className="text-center py-8 text-red-500">{error}</p>
                                ) : projects.length === 0 ? (
                                    <p className="text-center py-8">لا توجد أعمال لعرضها</p>
                                ) : (
                                    <div id="mesd">
                                        {projects.map(project => (
                                            <div key={project.id} className="flex-det">
                                                <div className="wrapper-01">
                                                                    ...
                                                                    {project.pictureUrls && project.pictureUrls.length > 0 ? (
                                                                        <>
                                                                            <img 
                                                                                ...
                                                                                src={"https://sani3ywebapiv1.runasp.net"+project.pictureUrls[0]} 
                                                                                alt="الصورة الرئيسية" 
                                                                            />
                                                                            ...
                                                                            <div className="floating-scroll">
                                                                                <div className="thumbnail-gallery">
                                                                                    {project.pictureUrls.map((img, index) => (
                                                                                        <img 
                                                                                            key={index} 
                                                    ...
                                                    <div className="wrapper-0">
                                                        ...
                                                        <div className="data-det">
                                                            <span>
                                                                تاريـخ انـجــاز الـمـشــروع: {new Date(project.dateJobDone).toLocaleDateString('ar-EG')}
                                                            </span>
                                                        </div>
                                                        <div className="flex gap-2 w-100">
                                                            <Link href={`/sanieey-dashboard/update-project?id=${project.id}`}>
                                                            <button 
