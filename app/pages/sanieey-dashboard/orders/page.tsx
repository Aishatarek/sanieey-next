...
const Page = () => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        switch (filter) {
          case "new":
            apiUrl =
              "https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/craftsman/new-orders";
            break;
          case "inProgress":
            apiUrl =
              "https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/orders-under-implementation";
            break;
          case "completed":
            apiUrl =
              "https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/orders-completed";
            break;
          default:
            apiUrl =
              "https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/craftsman/new-orders";
        ...
        const transformedData = data.map((order) => ({
          ...
          profession: "خدمة", // You might want to get this from API if available
          craftsman: order.clientFullName,
          date: new Date(order.startDate).toLocaleDateString("ar-EG", {
            year: "numeric",
            month: "long",
            day: "numeric",
          }),
          status: filter,
          details: order.serviceDescription,
  ...
  const tabs = [
    ...
    { key: "inProgress", label: "طلبات قيد التنفيذ", icon: "" },
    { key: "completed", label: "طلبات تم تنفيذها", icon: "" },
  ];
                  ...
                  <Link href="/sanieey-dashboard/orders">
                    <li className="btn0 active">
                      ...
                      >
                        <path
                          opacity="0.4"
                          d="M21.0406 7.69L12.0006 12.92L2.96063 7.69C3.36063 6.95 3.94063 6.3 4.59063 5.94L9.93063 2.98C11.0706 2.34 12.9306 2.34 14.0706 2.98L19.4106 5.94C20.0606 6.3 20.6406 6.95 21.0406 7.69Z"
                        />
                        <path
                          opacity="0.6"
                          d="M12.0006 12.9204V22.5004C11.2506 22.5004 10.5006 22.3404 9.93062 22.0204L4.59063 19.0604C3.38063 18.3904 2.39062 16.7104 2.39062 15.3304V9.67043C2.39062 9.03043 2.61062 8.33043 2.96062 7.69043L12.0006 12.9204Z"
                        />
            ...
            <div className="flex gap-4 p-4">
              <ul className="gap-4 p-4">
                {tabs.map((tab) => (
                  <li
                    key={tab.key}
                    className={
                      "btn0 cursor-pointer flex items-center gap-2 " +
              ...
              <div className='w-full md:w-8/12"'>
                <div className="space-y-4 p-4">
                  {loading ? (
                    <p className="text-center py-8">جاري تحميل البيانات...</p>
                  ) : filtered.length > 0 ? (
                    filtered.map((o) => (
                      <div key={o.id} className="order border p-4 rounded-lg">
                        <div className="order-number">
                          طلب رقم: <span>{o.id}</span>
                        </div>
                        <p className="order-details-p">
                          المهنة المطلوبة: <span>{o.profession}</span>
                        </p>
                        <p className="order-details-p">
                          اسم العميل: <span>{o.craftsman}</span>
                        </p>
                        <p className="order-details-p">
                          الموقع: <span>{o.location}</span>
                        </p>
                        ...
                        <Link
                          href={`/sanieey-dashboard/order-detail?id=${o.id}`}
                        >
                    ...
                    <p className="text-center py-8">لا توجد طلبات لعرضها.</p>
                  )}
                </div>
              </div>
            </div>
