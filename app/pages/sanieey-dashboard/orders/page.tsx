    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                switch (filter) {
                    case 'new':
                        apiUrl = 'https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/craftsman/new-orders';
                        break;
                    case 'inProgress':
                        apiUrl = 'https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/orders-under-implementation';
                        break;
                    case 'completed':
                        apiUrl = 'https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/orders-completed';
                        break;
                    default:
                        apiUrl = 'https://sani3ywebapiv1.runasp.net/api/CraftsmanOrders/craftsman/new-orders';
                }
        ...
        <div className='bg-white'>
            ...
            <div className="flex flex-wrap gap-4 p-4">
                ...
                <div className="w-full md:w-8/12">
                    <div className="nam"><span>إدارة الـطـلـبـات</span> <img src="/images/Fill 177.svg" alt="" /></div>
                    ...
                    <div className="flex gap-4 p-4">
                        <ul className="gap-4 p-4">
                            {tabs.map(tab => (
                                <li
                                    key={tab.key}
                                    className={"btn0 cursor-pointer flex items-center gap-2 " + (filter === tab.key ? 'active' : '')}
                                ...
                                {loading ? (
                                    ...
                                    filtered.map(o => (
                                        <div key={o.id} className="order border p-4 rounded-lg">
                                            ...
                                            <Link href={`/sanieey-dashboard/order-detail?id=${o.id}`}>
                                                    <button className="btn-submit mt-4">تفاصيل الطلب</button>
                                                </Link>
                                        </div>
                                    ))
                                ) : (
